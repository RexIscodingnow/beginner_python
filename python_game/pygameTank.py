...
import sys  # 有另外開資料夾存放 pygame 模組，就使用，沒有的不用加上這兩行
sys.path.append("modules")  # 從「叫什麼名稱的資料夾 (資料夾不是叫 modules 請更改成該存放資料夾名稱)」導入 pygame 模組
import pygame
import time
# 檔案導入 (by 圖片、音檔 一般來說...)
import os
...
# 我方坦克 & 雙方子彈 
gamer_tank = pygame.image.load(os.path.join("python_game/image", "玩家坦克.jpg")).convert()
tank_bullet = pygame.image.load(os.path.join("python_game/image", "子彈要使用的.jpg")).convert()
# 敵方坦克
simple_tank = pygame.image.load(os.path.join("python_game/image", "一般坦克.jpg")).convert()
...
# 敵方生成點
def oppsite_tank():
    new_rect_tank = [0, WIDTH/2 - 30, WIDTH]

    while 1:
        for i in new_rect_tank:
            if i == 1:
                time.sleep(2.0)
            if i == 2:
                time.sleep(2.0)

            return new_rect_tank[i]
...
while running:
    ...
    # 2. 更新遊戲資料
    all_sprites.update()
    pygame.sprite.groupcollide(shoot_bullets, other_tank, True, True)
